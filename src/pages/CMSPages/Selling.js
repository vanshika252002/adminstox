...
const Selling = ({ faqItems }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formvalid && formdata?.heading) {
      const content = {
        name: "selling_collectible",
        type: "selling_collectible",
  ...
  // useEffect(() => {
  //     const newErr = faqItems[0]?.content?.contentData?.map(item => {
  //         if (item.type === "content") {
  //             return { ...item, content: '' }; // Clear error content only
  //         } else if (item.type === "imagePath") {
  //             return { ...item, imagePath: '' }; // Clear error imagePath only
  //         } else {
  //             return item; // If the type is neither, return the item as is
  //         }
  //     });
  //     console.log("New Errrosss", newErr);

  // }, [faqItems,contentErrors]);

  return (
    <div className="container mt-3">
      <div className="d-flex justify-content-between">
        <h5 className="mb-2 fw-600 pt-2">Selling a collectible</h5>
        {isPermission && (
