...
import { create_faq_question, get_dpa_data } from "../../reduxData/cms/cmsAction";
...
const DPA = () => {
  ...
  const { dpadata } = useSelector((state) => state.cms);
  ...
  const handleSubmit = async (e) => {
    ...
    if (content) {
      ...
      const contentId = dpadata ? dpadata?.id : null;
      ...
      await get_dpa_data(localStorage.getItem("token"), "data_processing_agreement", dispatch);
    }
  };
  ...
  useEffect(() => {
    ...
    if (token) {
      get_dpa_data(token, "data_processing_agreement", dispatch);
    }
  }, [dispatch]);
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      ...
      if (
        dpadata?.content?.Content !== undefined &&
        dpadata?.content?.Content !== null
      ) {
        next.content = dpadata.content.Content;
      }

      if (Object.prototype.hasOwnProperty.call(dpadata, "status")) {
        next.status = normalizeStatus(dpadata.status);
      }
  ...
  return (
    <div className="mt-1">
      <div className="container">
        <h3 className="mb-0 fw-600">Data Processing Agreement</h3>
